...
public class EnemySet {
    ...
    public float getCollumnSpacing(){
    	if(maxCollumns > 0){
    		return (BaseCode.world.getWidth()-leftPadding-rightPadding)/maxCollumns;
    	}
    	return 0;
    }
    /**
    ...
    public float getRowSpacing(){
    	if(this.totalRows > 0){
    		return (BaseCode.world.getHeight()-topPadding-bottomPadding)/totalRows;
    	}
    	return 0;
    }
    /**
    ...
    public void setSetPadding(float top, float bottom, float left, float right){
    	if((top + bottom) <= BaseCode.world.getHeight() && (right + left) <= BaseCode.world.getWidth()){
    		topPadding = top;
     ...
     */
    public boolean moveEnemy(BaseEnemy moveTarget, int row, int collumn){
     ...
     */
    public boolean addEnemy(BaseEnemy newEnemy, int row, int collumn){
     ...
     */
    public boolean removeEnemy(BaseEnemy removeTarget){
     ...
     */
    public boolean moveRepresentative(InteractableObject newReprestitive, int row, int collumn){
     ...
     * @return
     */
    public boolean removeRepresentative(InteractableObject removeTarget){
     ...
     * @param collumn
     */
    public boolean getWorldPosition(Vector2 host, int row, int collumn){
    	if(host != null){
    		host.setX(leftPadding);
    		if(maxCollumns > 1){
    			host.setX(host.getX() + (((BaseCode.world.getWidth() - leftPadding - rightPadding) / (maxCollumns - 1)) * collumn));
    		}
    		else{
    			host.setX(host.getX() + ((BaseCode.world.getWidth() - leftPadding - rightPadding) * collumn));
    		}
    		host.setY(BaseCode.world.getHeight() - topPadding);
    		if(totalRows > 1){
    			host.setY(host.getY() - (((BaseCode.world.getHeight() - topPadding - bottomPadding) / (totalRows - 1)) * row));
    		}
    		else{
    			host.setY(host.getY() - ((BaseCode.world.getHeight() - topPadding - bottomPadding) * row));
    		}
    		return true;
    	}
    	return false;
    ...
    /**
     * Allows movement to all enemies in set for an amount of time.
     * Time is in update ticks.
     */
    	...
    	if(enemySetPrime != null){
    		for(int loopHeight = 0; loopHeight < enemySetPrime.length; loopHeight++){
    			if(enemySetPrime[loopHeight] != null){
    				for(int loopWidth = 0;  loopWidth < enemySetPrime[loopHeight].length; loopWidth++){
    					if(enemySetPrime[loopHeight][loopWidth] != null){
    	    				enemySetPrime[loopHeight][loopWidth].allowMovement(allowedTime);
    	    			}
    				}
    			}
    		}
    ...
    /**
     * Plays the introduction animations of any enemy waiting to
     * be introduced.
     */
    public void playIntroductions(int animTime)
    {
    	if(enemySetPrime != null){
    		for(int loopHeight = 0; loopHeight < enemySetPrime.length; loopHeight++){
    			if(enemySetPrime[loopHeight] != null){
    				for(int loopWidth = 0;  loopWidth < enemySetPrime[loopHeight].length; loopWidth++){
    					if(enemySetPrime[loopHeight][loopWidth] != null){
    	    				enemySetPrime[loopHeight][loopWidth].playWaitingIntroductions(animTime);
    	    			}
    				}
    			}
    		}
     ...
     * be introduced.
     */
    public StateChange activateSpecialAbilities(int animTime)
    {
    	storedStateChangeObject.setToDefualts();
    	if(enemySetPrime != null){
    		for(int loopHeight = 0; loopHeight < enemySetPrime.length; loopHeight++){
    			if(enemySetPrime[loopHeight] != null){
    				for(int loopWidth = 0;  loopWidth < enemySetPrime[loopHeight].length; loopWidth++){
    					if(enemySetPrime[loopHeight][loopWidth] != null){
    	    				enemySetPrime[loopHeight][loopWidth].setAnimationPosition(0);
    	    				storedStateChangeObject.add(enemySetPrime[loopHeight][loopWidth].activateAbility(animTime));
    	    			}
    				}
    			}
    		}
    			...
    			if(enemySetPrime[loopRow] != null){
	    			for(int loopCollumn = 0; loopCollumn < enemySetPrime[loopRow].length; loopCollumn++){
	    				...
	    				if(enemySetPrime[loopRow][loopCollumn] != null && !enemySetPrime[loopRow][loopCollumn].isEnemyCircled()){
	    					enemySetPrime[loopRow][loopCollumn].draw();
	    				}
	    			}
    			}	
    		}
     ...
     * @return
     */
    public boolean doesGridPositionExist(int row, int collumn){
     ...
     * @return
     */
    private boolean isActiveEnemy(BaseEnemy target){
    ...
    private boolean isRepresentativeActive(InteractableObject rep){
