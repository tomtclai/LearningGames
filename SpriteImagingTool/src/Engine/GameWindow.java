...
public abstract class GameWindow extends Applet implements Runnable
{
  ...
  private void gameCycle()
  {
    ...
    {
      numFramesSkipped += 1;
      //System.out.println("Loop time: " + delta + "; Threshold: " + FRAME_SKIP_THRESHOLD);
    }
  ...
  public void run2()
  {
    final long timeStep = FRAME_DELAY;
    ...
    while(!shouldClose)
    {
      ...
      // Update the number of time steps since the last update
      while((curTime - prevTime) >= timeStep)
      {
        updateWorld();
        updateInput();
  ...
  */
  ...
  private void cleanExit()
  {
    ...
    if(LOOP_OPTION == 2)
    {
      // OPTION_2
      /*
      // Wait for the update thread to end
    ...
    else if(this.getParent() != null)
    {
      ...
      {
        WindowEvent wev =
            new WindowEvent(parentFrame, WindowEvent.WINDOW_CLOSING);
   ...
   * 
   * Must be instancaited as a new Frame because setUndecorated() (removes topbar)
   * will fail once theFrame becomes displayable.
   */
