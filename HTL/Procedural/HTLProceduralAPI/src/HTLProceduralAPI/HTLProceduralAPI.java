...
public class HTLProceduralAPI extends HTL {
	...
	private float countdownCurrent = 0;
	...
	private void updateTimer() {
		if (countdownCurrent < 0.0) {
			countdownCurrent = (float) getCountdownMax();
			lastTimeTimerWasUpdated = System.currentTimeMillis();
		} else if (System.currentTimeMillis() - lastTimeTimerWasUpdated > 100) { // TODO:
																					// make
																					// 0.1
			countdownCurrent -= 0.1;
			lastTimeTimerWasUpdated = System.currentTimeMillis();
		}
	}
	...
	private void updateUI() {
		...
		if (countdownCurrent >= 0) {
			setHUDTime(new DecimalFormat("#.#").format(countdownCurrent));
		}
